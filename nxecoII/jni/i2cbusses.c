...
{
	int file;
	// __android_log_print(ANDROID_LOG_INFO,"JNI","Java_com_lt_jni_JNI_file %s",filename);
	...
	if (file < 0 && (errno == ENOENT || errno == ENOTDIR)) {
		...
		file = open(filename, O_RDWR);
		// __android_log_print(ANDROID_LOG_INFO,"JNI","Java_com_lt_jni_JNI_file %d",file);
	}
	...
	if (file < 0 && !quiet) {
		if (errno == ENOENT) {
			fprintf(stderr, "Error: Could not open file "
			 ...
			 __android_log_print(ANDROID_LOG_INFO,"JNI","Could not open file /dev/i2c-%d,i2c/%d,%s",i2cbus,i2cbus,strerror(ENOENT));
		} else {
			fprintf(stderr, "Error: Could not open file "
				"`%s': %s\n", filename, strerror(errno));
			 __android_log_print(ANDROID_LOG_INFO,"JNI","Java_com_lt_jni_JNI_file%s': %s\n",filename, strerror(errno));
			if (errno == EACCES)
